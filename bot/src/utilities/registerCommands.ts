...
const loadCommands = (baseDir = "../dist/commands/", dirDive = "") => {
  ...
  for (const file of readdirSync(p)) {
    ...
    if (fileStat.isFile() && file.endsWith(".js")) {
      ...
      if (!cmdReq) {
        console.warn(
          `"${baseDir}${dirDive}${file}" command does not export a default object`,
        );
        continue;
      }
      ...
      const { publicCommands, privateCommands } = loadCommands(
        baseDir,
        dirDive + `${file}/`,
      );
...
export default function (
  ...
  if (publicCommands.length >= 1) {
    const promise = rest.put(Routes.applicationCommands(config.clientID), {
...
export function clearCommands(local: boolean, config: ConfigFile) {
  return new Promise((resolve, reject) => {
    ...
    rest.put(route, { body: [] }).then(resolve).catch(reject);
  });
}
